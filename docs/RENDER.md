# 🚀 Render.com Deployment ve Yönetim

Bu doküman, KafkasDer Yönetim Paneli'ni Render.com'a deploy etme, MCP entegrasyonu ve webhook yapılandırmasını açıklar.

## 📋 İçindekiler

1. [Ön Gereksinimler](#ön-gereksinimler)
2. [Render Blueprint Kullanımı](#render-blueprint-kullanımı)
3. [Manuel Deployment](#manuel-deployment)
4. [Environment Variables](#environment-variables)
5. [Custom Domain](#custom-domain)
6. [Render MCP Server](#render-mcp-server)
7. [Webhook Yapılandırması](#webhook-yapılandırması)
8. [Monitoring ve Logging](#monitoring-ve-logging)
9. [Sorun Giderme](#sorun-giderme)

---

## ✅ Ön Gereksinimler

1. **Render.com Hesabı:**
   - [Render.com](https://render.com) üzerinde hesap oluşturun
   - GitHub/GitLab hesabınızı bağlayın

2. **Supabase Projesi:**
   - Supabase projeniz hazır olmalı
   - Database migration'ları tamamlanmış olmalı
   - Detaylar için [SUPABASE.md](./SUPABASE.md) dosyasına bakın

3. **Git Repository:**
   - Kod GitHub/GitLab'da olmalı
   - `render.yaml` dosyası root dizinde olmalı

---

## 📦 Render Blueprint Kullanımı

### Adım 1: Blueprint Dosyasını Kontrol Edin

`render.yaml` dosyası proje root'unda mevcut. Bu dosya Infrastructure as Code (IaC) yaklaşımıyla tüm servisleri tanımlar.

### Adım 2: Render Dashboard'da Blueprint Oluştur

1. [Render Dashboard](https://dashboard.render.com) → **New** → **Blueprint**
2. Repository'yi seçin: `Kafkasportal/Portal`
3. **Apply** butonuna tıklayın

### Adım 3: Environment Variables Ayarlayın

Render, `sync: false` olan environment variable'lar için sizden değer isteyecek:

---

## 🔧 Render MCP Server Entegrasyonu

**Render MCP Server**, AI geliştirme araçları (Cursor, Claude Code, GitHub Copilot) ile Render altyapınızı doğal dil komutlarıyla yönetmenizi sağlar.

### Özellikler

- ✅ **Servis Yönetimi**: Web servisleri, static siteler oluşturma
- ✅ **Veritabanı Yönetimi**: PostgreSQL veritabanları oluşturma ve sorgulama
- ✅ **Log Analizi**: Servis loglarını filtreleme ve analiz etme
- ✅ **Metrik İzleme**: CPU, memory, response time metriklerini görüntüleme
- ✅ **Deploy Takibi**: Deploy geçmişini ve detaylarını görüntüleme

### MCP Server Kurulumu

#### 1. Render API Key Oluşturma

1. [Render Dashboard](https://dashboard.render.com) → **Account Settings** → **API Keys**
2. **Create API Key** butonuna tıklayın
3. API key'i güvenli bir yerde saklayın

⚠️ **Güvenlik Uyarısı**: Render API key'leri geniş yetkilere sahiptir. Tüm workspace'lere ve servislere erişim sağlar.

#### 2. IDE Konfigürasyonu

##### Cursor Kurulumu

`~/.cursor/mcp.json` dosyasına ekleyin:

```json
{
  "mcpServers": {
    "render": {
      "url": "https://mcp.render.com/mcp",
      "headers": {
        "Authorization": "Bearer <YOUR_API_KEY>"
      }
    }
  }
}
```

##### VS Code (GitHub Copilot) Kurulumu

VS Code settings.json'a ekleyin:

```json
{
  "chat.mcp.servers": {
    "render": {
      "url": "https://mcp.render.com/mcp",
      "headers": {
        "Authorization": "Bearer <YOUR_API_KEY>"
      }
    }
  }
}
```

##### Claude Desktop Kurulumu

`~/Library/Application Support/Claude/claude_desktop_config.json` (macOS) veya `%APPDATA%\Claude\claude_desktop_config.json` (Windows):

```json
{
  "mcpServers": {
    "render": {
      "command": "npx",
      "args": [
        "mcp-remote",
        "https://mcp.render.com/mcp",
        "--header",
        "Authorization: Bearer ${RENDER_API_KEY}"
      ],
      "env": {
        "RENDER_API_KEY": "<YOUR_API_KEY>"
      }
    }
  }
}
```

#### 3. Workspace Ayarlama

MCP server'ı kullanmaya başlamak için workspace'inizi belirtin:

```
Set my Render workspace to Kafkasportal
```

### Örnek Kullanım Komutları

#### Servis Yönetimi

```
# Servisleri listele
List my Render services

# Yeni web servisi oluştur
Create a new web service named kafkasder-api using https://github.com/Kafkasportal/Portal

# Servis detaylarını görüntüle
Show details for kafkasder-panel service
```

#### Veritabanı İşlemleri

```
# Veritabanlarını listele
List my Render databases

# Yeni PostgreSQL veritabanı oluştur
Create a new database named kafkasder-prod with 10 GB storage

# SQL sorgusu çalıştır
Query my database: SELECT COUNT(*) FROM members WHERE active = true
```

#### Log ve Metrik Analizi

```
# Son hataları görüntüle
Show recent error logs for kafkasder-panel service

# CPU kullanımını kontrol et
What was the CPU usage for my service yesterday?

# Trafik analizi
Show me the busiest traffic day this month
```

#### Deploy Takibi

```
# Deploy geçmişini görüntüle
Show deploy history for kafkasder-panel

# Son deploy'un detayları
What happened in the latest deploy?
```

### MCP Server Sınırlamaları

- ❌ **Servis Silme**: Mevcut servisleri silemez
- ❌ **Free Instance**: Free tier instance'lar oluşturamaz
- ❌ **Tüm Servis Tipleri**: Sadece web service ve static site desteklenir
- ❌ **Deploy Tetikleme**: Manuel deploy tetikleyemez
- ❌ **Scaling Kontrolü**: Otomatik scaling ayarlarını değiştiremez

⚠️ **Güvenlik**: MCP server hassas bilgileri (connection string vb.) AI uygulamasının context'ine maruz bırakmamaya çalışır, ancak Render bunu garanti etmez.

**Zorunlu Değişkenler:**

- `NEXT_PUBLIC_APP_URL` - Production URL (örn: `https://panel.kafkasder.org`)
- `NEXT_PUBLIC_SUPABASE_URL` - Supabase project URL
- `NEXT_PUBLIC_SUPABASE_ANON_KEY` - Supabase anon key
- `SUPABASE_SERVICE_ROLE_KEY` - Supabase service role key (gizli)

**Opsiyonel Değişkenler:**

- `NEXT_PUBLIC_API_URL` - Eğer ayrı bir API varsa
- `NEXT_PUBLIC_GA_ID` - Google Analytics ID
- `NEXT_PUBLIC_SENTRY_DSN` - Sentry error tracking
- `RENDER_WEBHOOK_SECRET` - Webhook güvenlik secret'ı

### Adım 4: Deploy

Render otomatik olarak:

1. Repository'yi clone eder
2. `npm ci && npm run build` komutunu çalıştırır
3. `npm run db:migrate` komutunu çalıştırır (pre-deploy)
4. `npm start` komutu ile servisi başlatır

---

## 🔧 Manuel Deployment

Eğer Blueprint kullanmak istemiyorsanız:

### Adım 1: Yeni Web Service Oluştur

1. Render Dashboard → **New** → **Web Service**
2. Repository'yi bağlayın
3. Ayarları yapılandırın:

**Build Settings:**

- **Build Command:** `npm ci && npm run build`
- **Start Command:** `npm start`

**Advanced Settings:**

- **Pre-deploy Command:** `npm run db:migrate || true`
- **Health Check Path:** `/`
- **Auto-deploy:** `Yes` (veya `Only on merge to main`)

### Adım 2: Environment Variables Ekle

Manuel olarak tüm environment variable'ları ekleyin (yukarıdaki listeye bakın).

---

## 🔐 Environment Variables

### Production Environment Variables

```env
# Application
NODE_ENV=production
NEXT_PUBLIC_APP_NAME=KafkasDer Yönetim Paneli
NEXT_PUBLIC_APP_URL=https://panel.kafkasder.org

# Supabase
NEXT_PUBLIC_SUPABASE_URL=https://xxxxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
SUPABASE_SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...

# API
NEXT_PUBLIC_API_URL=https://api.kafkasder.org
NEXT_PUBLIC_API_TIMEOUT=30000
NEXT_PUBLIC_USE_MOCK_API=false

# Auth
NEXT_PUBLIC_AUTH_COOKIE_NAME=kafkasder-auth
NEXT_PUBLIC_AUTH_COOKIE_EXPIRES=7d

# Features
NEXT_PUBLIC_ENABLE_QR_SCANNER=true
NEXT_PUBLIC_ENABLE_EXCEL_EXPORT=true
NEXT_PUBLIC_ENABLE_DEVTOOLS=false

# Webhook
RENDER_WEBHOOK_SECRET=<auto-generated>
```

### Environment Variable'ları Render'da Ayarlama

1. Service → **Environment** sekmesi
2. **Add Environment Variable** butonuna tıklayın
3. Key ve Value'yu girin
4. **Save Changes** butonuna tıklayın

**⚠️ Önemli:** `SUPABASE_SERVICE_ROLE_KEY` gibi gizli bilgileri asla commit etmeyin!

---

## 🌐 Custom Domain

### Adım 1: Domain'i Render'a Ekle

1. Service → **Settings** → **Custom Domains**
2. **Add Custom Domain** butonuna tıklayın
3. Domain adını girin (örn: `panel.kafkasder.org`)

### Adım 2: DNS Ayarları

Render size DNS kayıtları verecek. Domain sağlayıcınızda şu kayıtları ekleyin:

**CNAME Kaydı:**

```
Type: CNAME
Name: panel (veya www)
Value: <render-service-url>.onrender.com
TTL: 3600
```

**A Kaydı (Root domain için):**

```
Type: A
Name: @
Value: <render-ip-address>
TTL: 3600
```

### Adım 3: SSL Sertifikası

Render otomatik olarak Let's Encrypt SSL sertifikası sağlar. Domain doğrulandıktan sonra HTTPS aktif olur.

---

## 🤖 Render MCP Server

Render.com'un resmi MCP (Model Context Protocol) server'ını Cursor'da kullanarak Render altyapınızı doğal dil komutlarıyla yönetin.

### Kurulum

#### Adım 1: API Key Oluşturma

1. [Render Dashboard](https://dashboard.render.com) → **Account Settings**
2. **API Keys** sekmesine gidin
3. **New API Key** butonuna tıklayın
4. Key'e bir isim verin (örn: "MCP Server")
5. Key'i kopyalayın ve güvenli bir yere kaydedin

**⚠️ Önemli:** API key'i sadece bir kez gösterilir. Kaydedin!

#### Adım 2: Cursor'da Yapılandırma

1. **MCP Konfigürasyon Dosyası:**
   - `.vscode/mcp.json` dosyası zaten oluşturulmuş
   - API key'i eklemeniz gerekiyor

2. **API Key'i Ekle:**

   ```json
   {
     "mcpServers": {
       "render": {
         "command": "npx",
         "args": [
           "-y",
           "@render-com/mcp-server"
         ],
         "env": {
           "RENDER_API_KEY": "YOUR_API_KEY_HERE"
         }
       }
     }
   }
   ```

3. **Cursor'ı Yeniden Başlat:**
   - Cursor'u kapatıp açın
   - MCP server otomatik olarak başlayacak

### Kullanım Örnekleri

```
"Render'daki tüm servislerimi listele"
"KafkasDer panel servisinin durumunu göster"
"Son deployment'ı göster"
"Deployment log'larını getir"
"CPU kullanımını göster"
"Memory kullanımını analiz et"
```

### Workspace Seçimi

MCP server'ı kullanmadan önce workspace'inizi seçmeniz gerekebilir:

```
"Set my Render workspace to kafkasder"
```

---

## 🔗 Webhook Yapılandırması

### Webhook Endpoint

Webhook endpoint'i şu adreste mevcuttur:

```
POST /api/webhooks/render
GET  /api/webhooks/render (health check)
```

**Production URL Örneği:**

```
https://your-domain.com/api/webhooks/render
```

### Render.com'da Webhook Oluşturma

#### Adım 1: Render Dashboard'a Git

1. [Render Dashboard](https://dashboard.render.com) → Webhooks
2. "New Webhook" butonuna tıkla

#### Adım 2: Webhook Ayarları

- **Name:** `KafkasDer Panel Deployments` (veya istediğiniz isim)
- **URL:** `https://your-domain.com/api/webhooks/render`
- **Events:** İstediğiniz event'leri seçin:
  - ✅ `Deploy Activated` - Deployment başarılı olduğunda
  - ✅ `Deploy Failed` - Deployment başarısız olduğunda
  - ✅ `Deploy Live` - Deployment canlıya alındığında
  - ✅ `Build Ended` - Build tamamlandığında

#### Adım 3: Secret Oluştur (Opsiyonel)

Güvenlik için bir secret oluşturun:

```bash
# Terminal'de random secret oluştur
openssl rand -hex 32
```

Bu secret'ı hem Render.com'da hem de environment variable olarak ekleyin:

```env
RENDER_WEBHOOK_SECRET=your-secret-here
```

### Event Tipleri

Webhook şu event tiplerini destekler:

**Deployment Events:**

- `deploy.activated` - Deployment aktif edildi
- `deploy.live` - Deployment canlıya alındı
- `deploy.failed` - Deployment başarısız oldu
- `deploy.created` - Yeni deployment oluşturuldu
- `deploy.canceled` - Deployment iptal edildi

**Build Events:**

- `build.started` - Build başladı
- `build.ended` - Build tamamlandı

### Test Etme

#### Health Check

```bash
curl https://your-domain.com/api/webhooks/render
```

Yanıt:

```json
{
  "message": "Render webhook endpoint is active",
  "timestamp": "2025-12-26T..."
}
```

#### Webhook Test

Render.com dashboard'dan "Test Webhook" butonuna tıklayarak test edebilirsiniz.

---

## 📊 Monitoring ve Logging

### Logs

1. Service → **Logs** sekmesi
2. Real-time log'ları görüntüleyin
3. Log'ları filtreleyin ve arayın

### Metrics

1. Service → **Metrics** sekmesi
2. CPU, Memory, Request rate gibi metrikleri görüntüleyin
3. Alert'ler ayarlayın

### Webhook Monitoring

Deployment event'lerini izlemek için webhook endpoint'ini kullanın:

- `/api/webhooks/render` endpoint'i deployment event'lerini dinler
- Slack/Discord bildirimleri ekleyebilirsiniz

---

## 🔄 CI/CD Pipeline

### GitHub Actions ile Otomatik Deploy

Render, GitHub Actions ile entegre çalışır:

1. **Auto-deploy ayarları:**
   - `autoDeployTrigger: checksPass` - CI check'leri geçtikten sonra deploy
   - `autoDeployTrigger: commit` - Her commit'te deploy
   - `autoDeployTrigger: off` - Manuel deploy

2. **Branch protection:**
   - `main` branch için `checksPass` kullanın
   - Feature branch'ler için `commit` kullanabilirsiniz

### Preview Environments

Pull request'ler için otomatik preview environment'lar oluşturulur:

```yaml
previews:
  generation: automatic
  expireAfterDays: 7
```

---

## 🐛 Sorun Giderme

### Build Başarısız Oluyor

1. **Log'ları kontrol edin:**
   - Service → **Logs** → Build log'larını inceleyin

2. **Yaygın sorunlar:**
   - `npm ci` hatası → `package-lock.json` commit edilmiş mi?
   - TypeScript hatası → `tsconfig.json` doğru mu?
   - Environment variable eksik → Tüm gerekli değişkenler ayarlı mı?

### Deployment Başarısız Oluyor

1. **Pre-deploy command:**
   - `npm run db:migrate || true` - Migration hataları deployment'ı durdurmaz

2. **Health check:**
   - `/` endpoint'i çalışıyor mu?
   - Health check timeout'u artırın

### Uygulama Çalışmıyor

1. **Environment variables:**
   - Tüm Supabase değişkenleri doğru mu?
   - `NEXT_PUBLIC_*` prefix'li değişkenler var mı?

2. **Database connection:**
   - Supabase RLS politikaları doğru mu?
   - Service role key doğru mu?

3. **Logs:**
   - Runtime log'larını kontrol edin
   - Error mesajlarını inceleyin

### MCP Server Başlamıyor

1. **API Key Kontrolü:**
   - `.vscode/mcp.json` dosyasında API key doğru mu?
   - Key'in geçerli olduğundan emin olun

2. **Cursor Yeniden Başlatma:**
   - Cursor'u tamamen kapatıp açın
   - MCP server otomatik başlamalı

### Webhook Çalışmıyor

1. ✅ Endpoint URL'i doğru mu kontrol edin
2. ✅ Environment variable'ları kontrol edin
3. ✅ Server loglarını kontrol edin
4. ✅ Render.com dashboard'da webhook durumunu kontrol edin

---

## 📈 Scaling

### Manuel Scaling

`render.yaml` dosyasında:

```yaml
numInstances: 2 # 2 instance çalıştır
```

### Autoscaling (Professional Workspace Gerekli)

```yaml
scaling:
  minInstances: 1
  maxInstances: 5
  targetCPUPercent: 70
  targetMemoryPercent: 70
```

---

## 🔒 Güvenlik

1. **Environment Variables:**
   - Gizli bilgileri asla commit etmeyin
   - `sync: false` kullanarak manuel giriş yapın

2. **Secrets:**
   - `RENDER_WEBHOOK_SECRET` otomatik generate edilir
   - Diğer secret'ları Render Dashboard'dan ekleyin

3. **IP Allow List:**
   - Supabase'de IP whitelist ayarlayın (gerekirse)
   - Render IP'lerini ekleyin

4. **Webhook Güvenliği:**
   - Production'da mutlaka `RENDER_WEBHOOK_SECRET` kullanın
   - Webhook URL'i mutlaka HTTPS olmalı

---

## 📝 Checklist

Deployment öncesi kontrol listesi:

- [ ] `render.yaml` dosyası commit edildi
- [ ] Tüm environment variable'lar ayarlandı
- [ ] Supabase migration'ları tamamlandı
- [ ] Health check endpoint çalışıyor
- [ ] Custom domain ayarlandı (opsiyonel)
- [ ] SSL sertifikası aktif
- [ ] Webhook endpoint test edildi
- [ ] Monitoring ve alert'ler ayarlandı
- [ ] Render MCP server yapılandırıldı (opsiyonel)

---

## 🔗 Faydalı Linkler

- [Render Documentation](https://render.com/docs)
- [Render Blueprint Reference](https://render.com/docs/blueprint-spec)
- [Next.js on Render](https://render.com/docs/deploy-nextjs)
- [Render MCP Documentation](https://render.com/docs/mcp-server)
- [Render API Documentation](https://api-docs.render.com)
- [Model Context Protocol](https://modelcontextprotocol.io)

---

**Son Güncelleme:** 26 Aralık 2025

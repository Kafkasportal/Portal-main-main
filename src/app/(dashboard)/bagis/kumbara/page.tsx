...
import { ScanQueueIndicator } from '@/components/features/kumbara/scan-queue-indicator'
import { CompactSyncButton } from '@/components/features/kumbara/manual-sync-button'
...
export default function KumbaraPage() {
  ...
  const { pendingCount, failedCount } = useScanQueueStore()
  const totalPendingScans = pendingCount + failedCount
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            {/* Queue Indicator - shows when there are pending scans */}
            {totalPendingScans > 0 && (
              <ScanQueueIndicator size="compact" variant="minimal" />
            )}

            {/* Sync Button - shows when there are pending scans and online */}
            {totalPendingScans > 0 && isOnline && (
              <CompactSyncButton />
            )}
          ...
          <CardContent className="space-y-1.5 p-3">
            <div className="flex items-start justify-between gap-1">
              ...
              <Badge
                ...
                className="text-2.5 shrink-0 px-1.5 py-0"
              >
                {statusLabels[kumbara.durum as keyof typeof statusLabels].label}
              </Badge>
            </div>
            <p className="text-muted-foreground text-2.5 flex items-center gap-1 truncate">

...
export default function BeneficiaryDetailPage({
  ...
  const { data: dependentPersons, isLoading: isLoadingDependents } = useQuery({
    ...
    enabled:
      !isNew && !Number.isNaN(id) && beneficiary?.tur === 'ihtiyac-sahibi-kisi',
  })
  ...
  const form = useForm<BeneficiaryFormData>({
    ...
    defaultValues: {
      ...
      uyruk: beneficiary?.uyruk || 'Türkiye',
      kategori: 'yetiskin',
      ...
      sponsorlukTipi: beneficiary?.sponsorlukTipi || 'yok',
      durum: defaultDurum || 'aktif',
      rizaBeyaniDurumu: beneficiary?.rizaBeyaniDurumu || 'alinmadi',
  ...
  const uploadMutation = useMutation({
    mutationFn: (file: File) =>
      uploadDocument(file, idParam, selectedDocType, setUploadProgress),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['documents', idParam] })
    ...
    <div className="relative space-y-4">
      ...
      <div className="pr-125">
        <h1 className="text-xl font-bold">
          İhtiyaç Sahibi Kişi ID # {String(data.id).slice(0, 8)}
        </h1>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          {/* Document List */}
          <DocumentList beneficiaryId={idParam} />
        </div>
      </LinkedRecordSheet>

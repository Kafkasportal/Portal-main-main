...
import { Download, Filter, RefreshCw, TrendingUp, Wallet } from 'lucide-react'
...
export default function NakdiIslemlerPage() {
  ...
  const stats = useMemo(() => {
    const nakdiIslemler = filteredData.filter((a: any) => a.yardimTuru === 'nakdi')
    return {
      toplam: nakdiIslemler.length,
      onaylanan: nakdiIslemler.filter((a: any) => a.durum === 'onaylandi').length,
      odenen: nakdiIslemler.filter((a: any) => a.durum === 'odendi').length,
      toplamTutar: nakdiIslemler.reduce((sum: number, a: any) => sum + (a.odemeBilgileri?.tutar || a.talepEdilenTutar || 0), 0),
    }
  }, [filteredData])
  ...
  if (isError) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="Nakdi İşlemler"
          description="Nakdi yardım işlemlerini takip edin ve yönetin"
        />
        <QueryError
          title="Veriler Yüklenemedi"
          message="Nakdi işlem verileri yüklenirken bir hata oluştu."
          onRetry={refetch}
        />
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">Nakdi yardım başvurusu</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.odenen}</div>
            <p className="text-muted-foreground text-xs">Tamamlanan ödeme</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      {isLoading ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle>Nakdi İşlem Listesi</CardTitle>
          </CardHeader>
          <CardContent>
            <DataTable
              columns={columns}

...
import { ScanQueueIndicator } from './scan-queue-indicator'
import { CompactSyncButton } from './manual-sync-button'
...
export function KumbaraToplamaDialog({
  ...
  // Initialize scan queue store on mount
  useEffect(() => {
    void initializeScanQueueStore()
  }, [])
  ...
  // QR Kod tarandığında - kumbarayı bul veya çevrimdışı kuyruğa ekle
  const handleQrScan = async (qrCode: string) => {
    ...
    try {
      ...
      if (!isOnline) {
        // Queue the scan for later syncing when offline
        if (!isDbAvailable) {
          toast.error('Çevrimdışı depolama kullanılamıyor', {
            description: 'Tarayıcınız gizli modda olabilir',
          })
          return
        }

        // Check for duplicate scans within the last 5 minutes
        const isDuplicate = await checkDuplicate(qrCode)
        if (isDuplicate) {
          toast.warning('Bu kumbara zaten tarandı', {
            description: 'Son 5 dakika içinde aynı QR kod tarandı',
          })
          return
        }

        // Add to offline queue
        const queuedScan = await addScanToQueue(qrCode, {
          deviceInfo: typeof navigator !== 'undefined' ? navigator.userAgent : undefined,
        })

        if (queuedScan) {
          toast.success('Tarama kuyruğa eklendi', {
            description: 'Bağlantı kurulduğunda otomatik olarak senkronize edilecek',
          })
          onOpenChange(false)
        } else {
          toast.error('Tarama kuyruğa eklenemedi', {
      ...
      // Online flow - fetch kumbara from server
      ...
      // If online fetch fails, try to queue offline if DB is available
      if (isDbAvailable) {
        const isDuplicate = await checkDuplicate(qrCode)
        if (!isDuplicate) {
          const queuedScan = await addScanToQueue(qrCode, {
          ...
          if (queuedScan) {
            toast.warning('Bağlantı hatası - tarama kuyruğa eklendi', {
              description: 'Bağlantı kurulduğunda otomatik olarak senkronize edilecek',
            })
            onOpenChange(false)
            return
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {isSearching ? (
            ...
            <div className="space-y-4 py-4">
              ...
              {!isOnline && (
                <div className="bg-warning/10 border-warning/20 flex items-center gap-3 rounded-lg border p-3">
                  <WifiOff className="text-warning h-5 w-5 flex-shrink-0" />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium">Çevrimdışı Mod</p>
                    <p className="text-muted-foreground text-xs">
                      Taramalar kuyruğa eklenir ve bağlantı kurulduğunda senkronize edilir
                    </p>
                  </div>
                </div>
              )}

              {/* Queue indicator and sync button */}
              {pendingCount > 0 && (
                <div className="flex items-center justify-between gap-2">
                  <ScanQueueIndicator size="compact" variant="minimal" />
                  {isOnline && <CompactSyncButton />}
                </div>
              )}

              <div className="bg-muted/50 flex flex-col items-center justify-center gap-4 rounded-lg p-8">
                {!isOnline ? (
                  <CloudOff className="text-warning h-16 w-16" />
                ) : (
                  <QrCode className="text-muted-foreground h-16 w-16" />
                )}
                <p className="text-muted-foreground text-center">
                  {!isOnline
                    ? 'Çevrimdışıyken taramalar kuyruğa eklenir'
                    : 'Toplamak istediğiniz kumbaranın QR kodunu tarayın'}
                </p>
                <Button
                  onClick={() => {
                ...
                >
                  ...
                  {!isOnline ? 'Kuyruğa Ekle için Tara' : 'Kumbarayı Tara'}
                </Button>
              </div>
            </div>
          ) : selectedKumbara ? (

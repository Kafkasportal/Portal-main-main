...
import { useCreateKumbara, useMembers } from '@/hooks/use-api'
...
export function YeniKumbaraDialog({
  ...
  // Fetch members for responsible person selection
  const {
    data: membersData,
  ...
  const { mutate, isPending } = useCreateKumbara({
    onSuccess: (_, variables) => {
      ...
      // Print preview için form verilerini kullan (bazı alanlar DB'de yok)
  ...
  const onSubmit = (data: KumbaraFormData) => {
    mutate({
      kod: data.qrKod,
      konum: data.konum,
      sorumlu_id: Number.parseInt(data.sorumluId, 10),

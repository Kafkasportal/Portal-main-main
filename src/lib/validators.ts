...
export const memberSchema = z.object({
  ...
  telefon: z
    ...
    .refine((val) => {
      ...
      return /^(\+90)?5[0-9]{9}$|^05[0-9]{9}$|^5[0-9]{9}$/.test(cleaned)
...
export const socialAidApplicationSchema = z.object({
  ...
  talepEdilenTutar: z.number().positive('Tutar pozitif olmalıdır').optional(),
  gerekce: z
    .string()
...
export type SocialAidApplicationFormData = z.infer<
  typeof socialAidApplicationSchema
>
...
export const kumbaraSchema = z.object({
  kod: z.string().min(3, 'Kumbara kodu en az 3 karakter olmalıdır'),
  konum: z.string().min(5, 'Konum en az 5 karakter olmalıdır'),
...
export const paymentSchema = z.object({
  beneficiaryId: z.number().min(1, 'İhtiyaç sahibi seçiniz'),
  ...
  odemeTarihi: z
    .string()
    .min(1, 'Ödeme tarihi gereklidir')
    .refine((val) => {
      if (!val || val === '' || val === 'invalid-date') return false
      const dateRegex = /^\d{4}-\d{2}-\d{2}$/
      if (!dateRegex.test(val)) return false
      const date = new Date(val)
      return !isNaN(date.getTime()) && date.toISOString().split('T')[0] === val
    }, 'Geçerli bir tarih giriniz (yyyy-MM-dd formatında)'),
  odemeYontemi: z.enum(['nakit', 'havale', 'elden'], {
    message: 'Ödeme yöntemi seçiniz',
  }),
  durum: z.enum(['beklemede', 'odendi', 'iptal'], {
...
export const receiptSchema = z.object({
  makbuzNo: z.string().min(1, 'Makbuz numarası gereklidir'),
  paymentId: z.number().min(1, 'Ödeme ID gereklidir'),
})
...
export const inKindAidSchema = z.object({
  beneficiaryId: z.number().min(1, 'İhtiyaç sahibi seçiniz'),
  yardimTuru: z.enum(['gida', 'giyim', 'yakacak', 'diger'], {
    message: 'Yardım türü seçiniz',
  }),
  miktar: z
    .number()
    .positive('Miktar pozitif bir sayı olmalıdır')
    .min(0.01, 'Minimum miktar 0.01'),
  birim: z.enum(['adet', 'kg', 'paket', 'kutu', 'takim', 'diger'], {
    message: 'Birim seçiniz',
  }),
  dagitimTarihi: z
    .string()
    .min(1, 'Dağıtım tarihi gereklidir')
    .refine((val) => {
      if (!val || val === '' || val === 'invalid-date') return false
      const dateRegex = /^\d{4}-\d{2}-\d{2}$/
      if (!dateRegex.test(val)) return false
...
export type InKindAidFormData = z.infer<typeof inKindAidSchema>

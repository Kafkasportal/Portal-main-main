...
test.describe('Bağış Modülü Testleri', () => {
  test.describe('Bağış Listesi Sayfası', () => {
    test('bağış listesi sayfasına gidilebilmeli', async ({
    ...
    test('bağış tablosu görünmeli', async ({ authenticatedPage: page }) => {
      ...
      // Tablo başlıkları
      await expect(
        page.getByRole('columnheader', { name: /Bağışçı|Ad/i })
      ).toBeVisible()
      await expect(
        page.getByRole('columnheader', { name: /Tutar/i })
      ).toBeVisible()
      await expect(
    ...
    test('durum filtresi çalışmalı', async ({ authenticatedPage: page }) => {
      ...
      const filterButton = page
        .getByRole('button', { name: /Durum|Filtre/i })
        .first()
      if (await filterButton.isVisible()) {
        ...
        await expect(
          page.getByRole('option', { name: /Tamamlandı|Beklemede/i }).first()
        ).toBeVisible()
      }
    })
  ...
  test.describe('Yeni Bağış Ekleme', () => {
    test("yeni bağış dialog'u açılmalı", async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.donations.list)
    ...
    test('bağış formu doğru alanları içermeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.donations.list)
  ...
  test.describe('Kumbara Sayfası', () => {
    test('kumbara sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Bağışlar' }).click()
    ...
    test('kumbara kartları veya tablosu görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.donations.kumbara)
  ...
  test.describe('Raporlar Sayfası', () => {
    test('raporlar sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Bağışlar' }).click()
  ...
  test.describe('Bağış Detay Görünümü', () => {
    test('bağış satırına tıklanınca detay görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.donations.list)

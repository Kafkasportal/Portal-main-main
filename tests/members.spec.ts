...
test.describe('Üye Modülü Testleri', () => {
  test.describe('Üye Listesi Sayfası', () => {
    test('üye listesi sayfasına gidilebilmeli', async ({
    ...
    test('üye tablosu görünmeli', async ({ authenticatedPage: page }) => {
      ...
      // Tablo başlıkları
      await expect(
        page.getByRole('columnheader', { name: /Ad|İsim/i }).first()
      ).toBeVisible()
      await expect(
        page.getByRole('columnheader', { name: /Soyad/i })
      ).toBeVisible()
    })
    ...
    test('üye arama çalışmalı', async ({ authenticatedPage: page }) => {
      ...
      // Arama sonuçlarının güncellenmesini bekle
    ...
    test('satır sayısı değiştirilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.list)
  ...
  test.describe('Yeni Üye Ekleme', () => {
    test('yeni üye sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Üyeler' }).click()
    ...
    test('üye formu görünmeli', async ({ authenticatedPage: page }) => {
      ...
      // Form başlığı
      await expect(
        page.getByRole('heading', { name: /Yeni Üye|Üye Ekle/i })
      ).toBeVisible()
    ...
    test('boş form gönderilince hata vermeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.new)
    ...
    test('geçerli verilerle üye eklenebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.new)
    ...
    test('telefon numarası formatı kontrol edilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.new)
      ...
      if (await phoneField.isVisible()) {
        ...
        await expect(
          page.getByText(/geçerli|telefon|format/i).first()
        ).toBeVisible()
      }
    })

    test('email formatı kontrol edilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.new)
      ...
      if (await emailField.isVisible()) {
        ...
        // Email format hatası
        await expect(
          page.getByText(/geçerli|e-posta|email|format/i).first()
        ).toBeVisible()
      }
    })
  })
  ...
  test.describe('Üye Detay', () => {
    test('üye satırına tıklanınca detay görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.list)
  ...
  test.describe('Üye Düzenleme', () => {
    test('üye düzenleme butonları görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.list)
  ...
  test.describe('Excel Export', () => {
    test('excel export butonu görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.members.list)

...
test.describe('Sosyal Yardım Modülü Testleri', () => {
  test.describe('İhtiyaç Sahipleri Listesi', () => {
    test('ihtiyaç sahipleri sayfasına gidilebilmeli', async ({
    ...
    test('ihtiyaç sahipleri tablosu görünmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.beneficiaries)

      await expect(
        page.getByRole('heading', { name: /İhtiyaç Sahip/i })
      ).toBeVisible()
    ...
    test('yeni ihtiyaç sahibi eklenebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.beneficiaries)
  ...
  test.describe('Başvurular Sayfası', () => {
    test('başvurular sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Sosyal Yardım' }).click()
    ...
    test('başvuru listesi görünmeli', async ({ authenticatedPage: page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: /Başvuru/i })
      ).toBeVisible()
    })

    test('başvuru durumu filtrelenebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.applications)

      const statusFilter = page
        .getByRole('combobox', { name: /Durum|Filtre/i })
        .first()
      if (await statusFilter.isVisible()) {
        ...
        // Durum seçenekleri
        const option = page
          .getByRole('option', { name: /Beklemede|Onaylandı|Reddedildi/i })
          .first()
    ...
    test('başvuru detayları görüntülenebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.applications)
  ...
  test.describe('Ödemeler Sayfası', () => {
    test('ödemeler sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Sosyal Yardım' }).click()
    ...
    test('ödeme tarih filtresi çalışmalı', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.payments)
  ...
  test.describe('İstatistikler Sayfası', () => {
    test('istatistikler sayfasına gidilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.getByRole('button', { name: 'Sosyal Yardım' }).click()
    ...
    test('istatistik kartları görünmeli', async ({
      authenticatedPage: page,
    }) => {
  ...
  test.describe('Yeni İhtiyaç Sahibi Formu', () => {
    test('form alanları doğru çalışmalı', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.beneficiaries)
    ...
    test('zorunlu alan kontrolü yapılmalı', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.beneficiaries)
      ...
      if (await addButton.isVisible()) {
        ...
        const submitButton = page
          .getByRole('button', { name: /Kaydet|Ekle/i })
          .last()
        if (await submitButton.isVisible()) {
          ...
          // Hata mesajı
          await expect(
            page.getByText(/zorunlu|gerekli|required/i).first()
          ).toBeVisible()
        }
      }
    })
  ...
  test.describe('Export İşlemleri', () => {
    test('ihtiyaç sahipleri export edilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.beneficiaries)

      const exportButton = page.getByRole('button', {
        name: /Export|Excel|İndir/i,
      })
    ...
    test('ödemeler export edilebilmeli', async ({
      authenticatedPage: page,
    }) => {
      await page.goto(ROUTES.socialAid.payments)
